...
import { Mail, Trash2, Calendar, User, MessageSquare, Send } from 'lucide-react';
...
export default function ContactsManager({ messages, refreshData }) {
  ...
  // Logic to open local email client
  ...
  return (
    <div className="p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-10 flex justify-between items-end">
        <div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">Public Inquiries</h2>
          <p className="text-slate-500 font-medium">Managing {messages.length} messages from the public.</p>
        </div>
      </div>
      ...
      {messages.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 bg-slate-50 rounded-[3rem] border-2 border-dashed border-slate-200">
          ...
          <p className="text-slate-400 font-bold uppercase text-xs tracking-widest">No messages found</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {messages.map((msg) => (
            <div key={msg._id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-md transition-all group relative">
